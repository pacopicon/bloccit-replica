class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*') # Select all attributes of the user
        ...
        .order('rank DESC') # Instruts the database to order the results in descending order, by the rank that we created in this query. (rank = comment count + post count)
  end
